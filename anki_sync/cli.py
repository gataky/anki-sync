...
def sync(sheet_id: str, sheet_name: str, deck_name: str, output_file: str, sound_dir: str) -> None:
    """
    ...
    try:
        ...
        if not words:
            click.secho("No words fetched or an error occurred during fetching. Exiting.", fg="yellow")
            return
        ...
        click.secho(f"An unexpected error occurred during the sync process: {e}", fg="red", err=True)
        import traceback
        click.secho(traceback.format_exc(), fg="red", err=True) # For more detailed debugging
