...
def _process_guid_for_row(
    guid_cell_value: Any, row_idx: int, sheet_name: str
...
def _prefix_article_to_greek_word(
    original_greek_word: str, gender_string_for_article: str
) -> str:
...
def _parse_sheet_row(
    padded_row: List[Any], row_idx: int, sheet_name: str, sound_dir: str
    ...
    """
    guid_to_use, guid_update_item = _process_guid_for_row(
    ...
    if sound_filename and original_greek_word and sound_dir and SYNTHESIZER.client:
        ...
        if not os.path.exists(full_sound_path):
            print(
                f"Attempting to synthesize audio for '{original_greek_word}' as it's missing at '{full_sound_path}'..."
            )
            # SYNTHESIZER.text expects the base word and the output directory
            SYNTHESIZER.text(
                text_to_synthesize=processed_greek_word, output_directory=sound_dir
            )
        # else:
    ...
    if (
        ...
        return (
            None,
            guid_update_item,
        )  # word_args is None, but guid_update_item might exist

    return word_args, guid_update_item


def _write_guids_to_sheet(
    sheet_api: Any, sheet_id: str, sheet_name: str, updates: List[Dict[str, Any]]
):
    ...
    try:
        ...
        print(
            "Note: The Word objects in memory will have the new GUIDs, but they were not saved back to the sheet."
        )
...
def get_words_from_sheet(sheet_id: str, sheet_name: str, sound_dir: str) -> List[Word]:
    ...
    for row_idx, row in enumerate(values):
        if not any(
            cell for cell in row if str(cell).strip()
        ):  # Skip entirely empty or whitespace-only rows
            continue
        try:
            ...
            print(
                f"Error processing row {row_idx + 2} (Sheet row). Content: {row}. Error: {e}"
            )
