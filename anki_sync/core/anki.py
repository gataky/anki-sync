...
ANKI_MODEL_TEMPLATES = [
    {
        ...
        'qfmt': '{{English}}',
        'afmt': '{{FrontSide}}<hr id="answer">{{Greek}}<br>{{Sound}}', # {{Sound}} will render as [sound:filename.mp3]
    },
    {
        'name': 'Card 2 (Greek to English)',
        'qfmt': '{{Greek}}<div style="font-size:0.8em; color:grey;">({{Class}}{{#Gender}} // {{Gender}}{{/Gender}})</div><br>{{Sound}}',
...
def create_anki_deck(words: list[Word], deck_name: str, output_abs_path: str, sound_files_dir: str | None = None) -> None:
    """
        ...
        if word.sound_file:
            if sound_files_dir:
                ...
                if os.path.exists(sound_file_path):
                    ...
                    print(f"Warning: Sound file '{word.sound_file}' not found in '{sound_files_dir}' for word GUID {word.guid}.")
            else:
                print(f"Warning: Sound file '{word.sound_file}' specified for word GUID {word.guid}, but no sound_files_dir provided. Sound will not be packaged.")

        note_fields = [
            word.guid, word.english, word.greek, word.word_class, word.gender or "", sound_field_value
        ]
        anki_note = genanki.Note(model=anki_model_instance, fields=note_fields, tags=word.tags)
